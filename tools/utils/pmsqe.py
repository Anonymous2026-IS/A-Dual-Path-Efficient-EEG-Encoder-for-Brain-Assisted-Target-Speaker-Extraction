...
import scipy.io
import scipy.signal
import tools.utils.perceptual_constants as perceptual_constants
...
def bark_frequency_equalization(bark_spectra_ref, bark_spectra_deg):
    ...
    ref_thresholded = tf.where(tf.greater_equal(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_ref,
                               tf.zeros(tf.shape(bark_spectra_ref)))
    deg_thresholded = tf.where(tf.greater_equal(bark_spectra_deg, abs_thresh_power * 100.0), bark_spectra_deg,
                               tf.zeros(tf.shape(bark_spectra_deg)))

    # Average Pow per bark bin in reference and degraded signals
    avg_pow_per_bark_ref = tf.reduce_sum(tf.where(not_silent, ref_thresholded, tf.zeros(tf.shape(bark_spectra_ref))),
                                         axis=0)
    avg_pow_per_bark_deg = tf.reduce_sum(tf.where(not_silent, deg_thresholded, tf.zeros(tf.shape(bark_spectra_deg))),
                                         axis=0)
...
def bark_frequency_equalization_on_degraded(bark_spectra_ref, bark_spectra_deg):
    ...
    ref_thresholded = tf.where(tf.greater_equal(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_ref,
                               tf.zeros(tf.shape(bark_spectra_ref)))
    deg_thresholded = tf.where(tf.greater_equal(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_deg,
                               tf.zeros(tf.shape(bark_spectra_deg)))
...
def init_constants(Fs=8000, Pow_factor=0, apply_SLL_equalization=True,
    ...
    if Fs == 16000:
        ...
        # Matrices
        mat = scipy.io.loadmat('utils/bark_matrix_16k.mat')  # Not nice with absolute path
