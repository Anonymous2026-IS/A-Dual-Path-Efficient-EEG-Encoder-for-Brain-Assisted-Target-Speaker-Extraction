...
from .mossformer import MossFormerMaskNet
...
class separation(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, layer, kernel=3,
        ...
        # for i in range(self.layer):
        #     self.separation.append(Dual_RNN_Block(hidden_dim, hidden_dim,
        #                              rnn_type=rnn_type, norm=norm, dropout=dropout,
        #                              bidirectional=bidirectional))
                    
        # self.conv2d = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1)
        self.prelu = nn.PReLU()
    ...
    def forward(self, input, spike):
        ...
        # input = torch.cat((input,spike), dim=-1)
        input = self.fusion(input, spike)
        # print("Input shape:",input.shape)# ([1, 64, 1624])
        
        # audio, gap = self._Segmentation(input, self.K)  #(8,64,250,60)

        # for i in range(self.layer):
        #     audio = self.separation[i](audio)    
         #x: Input tensor of dimension [B, N, S].
        #out: Output tensor of dimension [spks, B, N, S]

        audio =  self.mask_net(input) 
        # print("audio shape:",audio.shape) 
        output = audio[0] 
        output = self.prelu(output)
        output = self.end_conv1x1(output)  # 调整通道数
        output = self.activation(output)
        # audio = self.conv2d(audio)
        # B, _, K, S = audio.shape
        # audio = audio.view(B,-1, K, S)
        # # [B*spks, N, L]
        # output = self._over_add(audio, gap)  #(8,64,7315)
        # #output = self.output(output)*self.output_gate(output)        
        # output = self.prelu(output)
