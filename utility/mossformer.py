...
class GlobalLayerNorm(nn.Module):#全局层归一化
...
class CumulativeLayerNorm(nn.LayerNorm):#对通道维进行归一化
...
def select_norm(norm, dim, shape):#选择归一化函数
...
class Encoder(nn.Module):#1D 卷积特征提取器，用于把一维音频序列转换成多通道的表示
...
class Decoder(nn.ConvTranspose1d):# Encoder 的对偶结构，用于将编码后的时序特征图还原回近似的原始波形（1D音频）
