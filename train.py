...
def train(num_gpus, rank, group_name,
    ...
    # predefine model
    net = DP_EEG_TSE(L1=network_config["L1"], L2=network_config["L2"], L3=network_config["L3"], enc_channel=network_config["enc_channel"], 
    ...
    #     optimizer,
    #     mode="min",  # 监测指标的模式，"min" 表示当监测指标停滞时降低学习率
    #     factor=0.5,  # 学习率降低的倍数，新学习率 = 旧学习率 * factor
    #     patience=5,  # 在指标停滞多少个 epoch 后降低学习率
    #     min_lr=1e-8,   # 学习率的下限，不会低于这个值
    #     verbose=False)    # 是否打印学习率调整的信息
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='configs/DP_EEG_TSE.json',
                        help='JSON file for configuration')
    parser.add_argument('-r', '--rank', type=int, default=0,
                        help='rank of process for distributed')
    parser.add_argument('-g', '--group_name', type=str, default='default_group',
                        help='name of group for distributed')
    args = parser.parse_args()
    #args.config= 'configs/DP_EEG_TSE.json'
